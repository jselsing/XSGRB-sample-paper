...
def main():
    ...
    # pl.step(np.sort(nH_tot), np.arange(np.sort(nH_tot).size)/len(nH_tot)) 
    # pl.step(np.sort(OA[:, 2].astype("float")), np.arange(np.sort(OA[:, 2].astype("float")).size)/len(OA[:, 2].astype("float"))) 
    # pl.show()
    ...
    # pl.step(np.sort(nH), np.arange(np.sort(nH).size)/len(nH))
    ...
    #         ax.add_artist(Ellipse((kk, ll), 2*jj*nH_std[sort][ii], 2*jj*OA_nHe[sort_OA][ii], fill=False, linestyle='dashed', lw = 0.5, alpha = 1.0/(2.0*jj) , color=cmap(OA_z[sort_OA][ii]/max(OA_z[sort_OA]))))


    for ii, txt in enumerate(names[sort]):
        ax.annotate(txt, (nH[sort][ii], OA_nH[sort_OA][ii]), size="x-small")
