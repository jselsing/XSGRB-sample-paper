...
def load_array(data_array):
    ...
    trans = f(data_array[:, 1])


    if thomas:
        trans = f(data_array[:, 0])
        if np.sum((data_array[:, 0] < 4000).astype("int")) != 0:
            trans = np.ones_like(trans)
        tellcorr = 1 / trans
        return data_array[:, 0], data_array[:, 1]*tellcorr, data_array[:, 2]*tellcorr, np.zeros_like(trans)

    has_resp = False
    if np.nanmean(data_array[:, 6]) < 1e-10:
        has_resp = True

    if np.sum((data_array[:, 1] < 4000).astype("int")) != 0:
        trans = np.ones_like(trans)
...
def stitch_XSH_spectra(waves, fluxs, errors, bpmaps, scale=None):
    ...
    try:
        ...
        print("Fitting failed ... Skipping NIR scaling")
        off_NIR = 1
...
def main():
    ...
    pl.plot(wl_bin, medfilt(flux_bin, 41), linestyle="steps-mid", lw=2.0, alpha=0.7)
