...
limit on the redshift distribution, we have shown the BAT sensitivity limit to
$\sim 1$ s flux ($\sim 3 \times
10^{-8}~\mathrm{erg}~\mathrm{s}^{-1}~\mathrm{cm}^{-2}$;
\citealt{Baumgartner2013, Lien2016}), also in the figure. Due to complex
triggering mechanism of \swift, this sensitivity limit should be interpreted
with some caution as effective limit is depends on the light curve of the prompt
emission signal. Due to the dilution of light with distance, the \swift GRB
luminosity detection limit is almost an order of magnitude brighter at $z=2$
compared to $z=1$. At $z\geq4$ we are only able to observe GRBs that are hundred
times brighter than the faintest bursts at $z=1$ and below.
