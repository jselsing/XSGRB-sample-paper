...
associated hosts and represents \textit{all} observations of GRBs and their
hosts carried out by VLT/X-shooter from the commissioning of the instrument,
\startdate, and until the end of the last period of the program 098.A-0055,
\termdate and thus constitutes 8 years of GRB observations with VLT/X-shooter.
An overview of all the spectra provided here are given in Table.
\ref{sample_overview}. For each burst, each individual observation is provided
...
\begin{figure*}
	\centering \includegraphics[width=16cm]{figures/XSGRB_zBATlum.pdf}
\caption{Redshift distribution as a function of intrinsic BAT $\gamma$-ray
	...
	X-shooter. For comparison, we overplot with red stars the GRB, quasar, and
	galaxy with the highest spectroscopically confirmed redshifts, the latter two
	shown at arbitrary $L_{\mathrm{BAT}}$. The blue solid line represents so-called
	1 s BAT sensitivity limit described in the text. The estimated epoch of
...
\nx~compared to the optically brights bursts. This indicates along with
...
It is clear that the fraction of dark bursts among for which we have taken a
spectrum is lower than the intrinsic dark burst fraction. Furthermore, correctly
associating a galaxy with a burst when there is no detected optical afterglow is
more difficult, see \citet{Jakobsson2005, Levesque2010} and \citet{Perley2017}
for a recent example. The larger fraction of upper limits on $\beta_{OX}$ for
increased darkness is evident both in the sample presented here and the one in
\citet{Fynbo2009}. 


As can be seen, the X-ray properties for the followed sample
does not differ significantly from the statistical sample, as shown in Sect.
\ref{completeness}. This mainly reflects the independence of the X-ray
...
but this number also includes bursts where no optical telescopes were available,
so is a lower limit. For the bursts entering our statistical sample, this number
is $\sim$ 27 percent, close to the maximum fraction of dark bursts
\citep{Melandri2012}. Of the subsample (3) that have actually been attempted,
this number is $\sim$ 18 percent, confirming that we are indeed biased against
bursts with an optical afterglow in the spectroscopic sample.
