...
our sample is biased against the overall \textit{Swift} sample. The values are
queried from then Three main samples are of interest to compare to assess the
completenessof the follow-up campaign; the full \textit{Swift} sample consisting
of all the burst observed by \textit{Swift} (1), the burst that fulfill the
selection criteria imposed in Sect. \ref{samplecrit} (2), and the bursts
actually followed up with X-shooter (3).

\input{tables/sample_properties.tex}

Using a
2-sided Kolmogorov-Smirnov (KS), we can assess the degree to which the null that
the two distribution are drawn from the same distribution can be rejected.
