...
%DIF DEL XSGRB_sample.tex        Tue Feb 20 16:08:16 2018
%DIF ADD XSGRB_sample_rev1.tex   Tue Oct 16 13:56:01 2018
...
\DIFdelbegin %DIFDELCMD < \abstract{In this work we present spectra of all $\gamma$-ray burst (GRB)
%DIFDELCMD < 	afterglows that have been promptly observed with the X-shooter spectrograph
%DIFDELCMD < 	until \termdate. In total, we obtained spectroscopic observations of 103 individual GRBs
%DIFDELCMD < 	observed within 48 hours of the GRB trigger. Redshifts have been measured for 97
%DIFDELCMD < 	per cent of these, covering a redshift range from 0.059 to 7.84. Based on a set
%DIFDELCMD < 	of observational selection criteria that minimize biases with regards to
%DIFDELCMD < 	intrinsic properties of the GRBs, the follow-up effort has been focused on
%DIFDELCMD < 	producing a homogeneous sample of 93 afterglow spectra for GRBs discovered by
%DIFDELCMD < 	the {\em Swift} satellite. We here provide a public release of all the reduced
%DIFDELCMD < 	spectra, including continuum estimates and telluric absorption corrections. For
%DIFDELCMD < 	completeness, we also provide reductions for the 18 late-time observations of
%DIFDELCMD < 	the underlying host galaxies. We provide an assessment of the degree of
%DIFDELCMD < 	completeness with respect to the parent GRB population, in terms of the X-ray
%DIFDELCMD < 	properties of the bursts in the sample and find that the sample presented here
%DIFDELCMD < 	is representative of the full \textit{Swift} sample. We constrain the fraction
%DIFDELCMD < 	of dark bursts to be < 28 per cent and we confirm previous results that higher
%DIFDELCMD < 	optical darkness is correlated with increased X-ray absorption. For the 42
%DIFDELCMD < 	bursts for which it is possible, we provide a measurement of the neutral
%DIFDELCMD < 	hydrogen column density, increasing the total number of published H{\sc I}
%DIFDELCMD < 	column density measurements by $\sim$ 33 per cent. This dataset provides a
%DIFDELCMD < 	unique resource to study the ISM across cosmic time, from the local progenitor
%DIFDELCMD < 	surroundings to the intervening universe.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \abstract{In this work we present spectra of all $\gamma$-ray burst (GRB)
	afterglows that have been promptly observed with the X-shooter spectrograph
	until \termdate. In total, we obtained spectroscopic observations of 103 individual GRBs
	...
	of dark bursts to be < 28 per cent and we confirm previous results that higher
...
\DIFdelbegin \DIFdel{\citep{LIGOScientificCollaboration2017a, LIGOScientificCollaboration2017}}\DIFdelend \DIFaddbegin \DIFadd{\citep{2017ApJ...848L..14G, 2017ApJ...848L..15S, 2017ApJ...848L..16S,
	...
	2017Sci...358.1583K, 2017Sci...358.1556C, LIGOScientificCollaboration2017a,
	LIGOScientificCollaboration2017}}\DIFaddend .
...
\begin{enumerate}
	...
	\item GRB trigger by BAT \DIFdelbegin \DIFdel{onboard }\DIFdelend \DIFaddbegin \DIFadd{on-board }\DIFaddend the \swift~satellite
...
\DIFaddbegin \DIFadd{We remark, however, that while the selection of the afterglows has been carried
...
\begin{enumerate}
	\item The GRB triggered \textit{Swift}\DIFdelbegin \DIFdel{onboard}\DIFdelend .
	...
	\item The Sun elevation from \DIFdelbegin \DIFdel{Paranl }\DIFdelend \DIFaddbegin \DIFadd{Paranal }\DIFaddend is $< -12^\circ$ (both at trigger time and 15 min after).
...
pivotal \DIFdelbegin \DIFdel{exampl }\DIFdelend \DIFaddbegin \DIFadd{example }\DIFaddend is the study of temporal variability of GRB afterglow absorption
systems due to effect of the GRB itself on the \DIFdelbegin \DIFdel{sorrounding }\DIFdelend \DIFaddbegin \DIFadd{surrounding }\DIFaddend medium \citep[e.g.,
...
was GRB~091018, which is the first burst entering our statistically \DIFdelbegin \DIFdel{homogeneous
}\DIFdelend \DIFaddbegin \DIFadd{homogeneously selected
}\DIFaddend sample. For all bursts that fulfill our sample selection criteria, described in
...
\DIFaddend The remaining offset is corrected by cross-correlating the observed sky spectrum
...
\DIFaddbegin \DIFadd{The resulting offsets are generally small, on the order of $\sim 5$~km~s$^{-1}$,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \subsection{\DIFadd{Slit loss correction and flux calibration accuracy}} \label{fluxes}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\DIFadd{For the majority of the observations presented in this sample, the slit width of
...
conditions delivering a seeing PSF with a FWHM of 0}\farc9 \DIFadd{observed with a
0}\farc9 \DIFadd{slit, only 76.1 per cent of the light will enter the slit. This slit loss
is not corrected by the flux calibration, as flux standard stars are always
observed with a 5}\arcsec{} \DIFadd{slit. As a consequence, almost all spectra will,
without slit loss correction, underestimate the flux by some fraction.
Additionally, slit losses are wavelength dependent due to the wavelength
dependence of seeing \citep{Boyd1978} and as the seeing changes across the slit
in wavelength, so does the slit loss. This artificially introduces a change in
continuum slope.
}

\begin{figure}[!t]
	\centerline{\includegraphics[width=\columnwidth]{figures/flux_accuracy}}
\caption{\DIFaddFL{The left panel shows the acquisition camera magnitudes as a function
	of synthetic photometry derived from the spectra. The blue, dashed line
	represent equality between the two measures. The right panel shows a histogram
	of the differences between the two. The vertical lines in the histogram
	indicates the 16th, 50th, and 84th percentiles of the distribution ($\Delta
	\mathrm{mag} = 0.07 ^{+0.51} _{-0.45} $). This shows the accuracy of the flux
	calibration. The colouring of the data-points shows the photometric
	passband used by the acquisition camera, as indicated by the labels. The $R$- and $I$- band magnitudes have been calibrated against the USNO-B1 catalog,
	which has a poor photometric accuracy; this contributes significantly to the  observed scatter, and for this reason we indicate these data with a light color.}} \label{fig:flux_acq}
\end{figure}

\DIFadd{To correct for the slitlosses, we develop an algorithm that can, based on the
...
2D Moffat profile which we integrate over the used slit width. This can be used to correct for the estimated slit losses at each wavelength bin.
}

\DIFadd{To assess the accuracy of the flux-calibration, after the slit loss correction,
we compare a subset of afterglow spectra with photometry derived from the
acquisition camera. The acquisition camera takes a short image just before
the spectroscopic integration, which only introduces a minimal delay and allows us to secure photometry for the vast majority of events. As photometric flux standards are rarely acquired, we resort to sky surveys such as the USNO catalog (for the $R$ and $I$ bands; \citealt{Monet2003}) or SDSS and Pan-STARRS ($griz$; \citealt{Abolfathi2018,Chambers2016}), where available.
}

\DIFadd{To compare the photometry with the spectral flux calibration,
...
\DIFadd{Similar to what is found in \citet{Lopez2016}, there is in some cases a mismatch
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddend \subsection{Telluric correction} \label{tell_corr}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\DIFaddbegin \DIFadd{The effect of telluric correction varies greatly depending on the wavelength
...
\DIFaddend \begin{figure*}
	\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=\linewidth]{figures/GRB121024A.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.85\linewidth]{figures/GRB121024A.pdf}}
	\DIFaddendFL \caption{Telluric corrected, normalized spectrum of GRB~121024A at $z = 2.300$
...
\DIFdelbegin \DIFdel{Using the masked spectrum, the entire process is then repeated 500 times where
...
shown in Fig. \ref{fig:spectrum}. In the }%DIFDELCMD < \lya%%%
\DIFdel{~forest, where there is very little
flux at the continuum level, the performance of the normalization algorithm
depends on the continuum coverage redwards of the }%DIFDELCMD < \lya%%%
\DIFdel{~line. In some cases, very
...
}%DIFDELCMD < \url{https://github.com/jselsing/XSGRB-sample-paper}%%%
\DIFdel{.
                                                                                           }\DIFdelend %DIF > Using the masked spectrum, the entire process is then repeated 500 times where
%DIF > the final continuum estimate is the mean of the continuum realizations and the
%DIF > associated error estimate is the standard deviation. This error reflects the
%DIF > stability of the algorithm across the spectrum. An example of the performance is
%DIF > shown in Fig. \ref{fig:spectrum}. In the \lya~forest, where there is very little
%DIF > flux at the continuum level, the performance of the normalization algorithm
%DIF > depends on the continuum coverage redwards of the \lya~line. In some cases, very
%DIF > little continuum is contained in a single arm and a manual continuum estimate is
%DIF > provided, similar to what is done in \citet{Lopez2016}. In these cases the
%DIF > continuum error is set to 10 per cent. The code for the continuum estimate is
%DIF > released along with the paper at
%DIF > \url{https://github.com/jselsing/XSGRB-sample-paper}.
...
characteristics, e.g., \DIFdelbegin \DIFdel{curios }\DIFdelend \DIFaddbegin \DIFadd{curious }\DIFaddend properties of their light curves, their
...
ensure that the GRBs entering our \DIFdelbegin \DIFdel{homogeneous }\DIFdelend \DIFaddbegin \DIFadd{homogeneously selected }\DIFaddend sample, fairly represent the
...
\DIFaddbegin \DIFadd{A possible complication is that a large fraction of the redshifts are 
...
\DIFadd{First, numerous works, including this one, have found that GRB 
absorption systems have much higher column densities than randomly 
selected sight lines 
\citep{2001ApJ...549L.209G,Prochaska2007, Fynbo2009}. 
This fact itself shows that the bulk of GRB absorption lines are not due 
to random intervening systems, but are peculiar to the GRB host. One 
second line of evidence comes from the systematic detection of nebular 
emission lines at the same redshift as the absorption system. 
\citet{Kruhler2015} report a 94\% detection rate of emission 
lines at $z < 1$ (where sensitivity is not an issue, even in the 
presence of a bright afterglow), always in agreement with absorption. 
Besides, it is extremely rare to detect emission from QSO intervening 
systems (see e.g. \citealt{2010MNRAS.408.2128F} for a discussion), which 
again indicates that GRB absorption systems are native to their hosts. 
An even more direct determination comes from the detection, in several 
cases, of fine-structure and metastable lines (for example from Fe and 
Ni), which are exclusively detected in GRB environments (and have been 
directly connected to the UV field generated by the GRB itself; e.g. 
\citealt{2006ApJ...648...95P, Vreeswijk2007, DElia2009}). 
Finally, at $z \gtrsim 2$ (where Ly$\alpha$ enters the observed range) 
the onset of the Ly$\alpha$ forest also provides an \textit{upper} limit 
on the GRB redshift, which again is always consistent with the 
absorption line redshift. The whole set of these arguments indicates 
that in only very few cases, if any at all, is a misidentification of 
the redshift with the intervening system a concrete possibility. 
}


\DIFaddend \input{tables/redshift_comparison}
...
star-forming regions in their hosts. \DIFaddbegin \DIFadd{When comparing the GRB DLAs and and QSO DLAs we correct for the fact that, on average, the GRB light only goes though half the DLA. In Fig. \ref{fig:NH_dist}, we have therefore multiplied the inferred $N_{\mathrm{HI}}$ values by 2 \citep{Prochaska2008}. 
}\DIFaddend 
...
\caption{Distributions of hydrogen column densities for absorbers found in
	...
	\citet{Fynbo2009}. \DIFaddbeginFL \DIFaddFL{To correct for geometric differences between GRB DLAs and QSO DLAs, we have multiplied the inferred GRB DLA $N_{\mathrm{HI}}$ by 2.}\DIFaddendFL } \label{fig:NH_dist}
\end{figure}
...
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=1, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=1, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption{Measurements of the hydrogen column densities for all bursts with a
		...
		with dashed lines. \DIFaddbeginFL \DIFaddFL{The pressence of }\lya\DIFaddFL{, as noted in Tab. \ref{tab:HI}, is indicated with a black arrow.}\DIFaddendFL }
	\label{fig:HI1}
\end{figure*}
\clearpage
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=2, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=2, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption*{Fig. \ref{fig:HI1}. continued.}
	\label{fig:HI2}
\end{figure*}
\clearpage
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=3, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=3, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption*{Fig. \ref{fig:HI1}. continued.}
	\label{fig:HI3}
\end{figure*}
\clearpage
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=4, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=4, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption*{Fig. \ref{fig:HI1}. continued.}
	\label{fig:HI4}
\end{figure*}
\clearpage
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=5, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=5, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption*{Fig. \ref{fig:HI1}. continued.}
	\label{fig:HI5}
\end{figure*}
\clearpage
\begin{figure*}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[page=6, width=\linewidth]{figures/HI_measurements.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[page=6, width=0.85\linewidth]{figures/HI_measurements.pdf}
	\DIFaddendFL \caption*{Fig. \ref{fig:HI1}. continued.}
	\label{fig:HI6}
\end{figure*}
\clearpage
...
\section{The complex error function and the \DIFdelbegin \DIFdel{Voigt profile}\DIFdelend \DIFaddbegin \DIFadd{spectral PSF}\DIFaddend } \label{voigt}
...
Observed during \DIFdelbegin \DIFdel{paranalization }\DIFdelend \DIFaddbegin \DIFadd{the verification }\DIFaddend of the instrument, these data forms the basis of
...
before the instrument was \DIFdelbegin \DIFdel{science-verified}\DIFdelend \DIFaddbegin \DIFadd{finished completely}\DIFaddend , it does not enter into the
statistical sample.

\subsection{GRB~090809 (z = 2.737)} \label{090809}

Observed during the first \DIFdelbegin \DIFdel{science verification }\DIFdelend \DIFaddbegin \DIFadd{instrument testing }\DIFaddend period and was reported in GCN
...
Obtained during the second \DIFdelbegin \DIFdel{science
verification }\DIFdelend \DIFaddbegin \DIFadd{instrument testing }\DIFaddend period, this dataset forms the
...
The first burst observed during normal operation \DIFdelbegin \DIFdel{after science verification was
completed }\DIFdelend and the first burst that enters
...
started 4 hours after the \textit{Swift} trigger, \DIFdelbegin \DIFdel{totaling }\DIFdelend \DIFaddbegin \DIFadd{totalling }\DIFaddend 2.4 ks. Absorption
...
carried out in RRM starting 11 \DIFdelbegin \DIFdel{min }\DIFdelend \DIFaddbegin \DIFadd{minutes }\DIFaddend after the GRB trigger. See section \ref{RRM},
...
to $\sim$ 580 nm, offset by 2.5\arcsec~relative to the \DIFdelbegin \DIFdel{centering }\DIFdelend \DIFaddbegin \DIFadd{centring }\DIFaddend of the slit. No
...
continuum is visible across all three \DIFdelbegin \DIFdel{spectrscopic }\DIFdelend \DIFaddbegin \DIFadd{spectroscopic }\DIFaddend arms of VLT/X-shooter with
...
the VIS and NIR arm and \ha~is located in the middle of the $JH$\DIFdelbegin \DIFdel{-bandgap }\DIFdelend \DIFaddbegin \DIFadd{-band-gap }\DIFaddend and is
...
the background to be negative in the \DIFdelbegin \DIFdel{center }\DIFdelend \DIFaddbegin \DIFadd{centre }\DIFaddend of the \lya~trough.
...
absorption features \DIFdelbegin \DIFdel{identifed }\DIFdelend \DIFaddbegin \DIFadd{identified }\DIFaddend as due to \feii~and \mgii~as well as emission
...
\DIFdelbegin \DIFdel{addtionally }\DIFdelend \DIFaddbegin \DIFadd{additionally }\DIFaddend forms the basis for GCN 15645 \citep{GCN15645}. This spectrum,
...
GRB trigger \DIFdelbegin \DIFdel{onboard }\DIFdelend \DIFaddbegin \DIFadd{on-board }\DIFaddend \swift, this observation lasted $14\times600$ s for a total
...
trough \DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend at $\sim$ 370 nm due to \lya~signals the redshift of the GRB,
...
ending with a nodding sequence, \DIFdelbegin \DIFdel{totaling }\DIFdelend \DIFaddbegin \DIFadd{totalling }\DIFaddend $\sim$ 5000 s, are combined to form
...
association is additionally ambiguous for this, likely \DIFdelbegin \DIFdel{extincted }\DIFdelend \DIFaddbegin \DIFadd{reddened }\DIFaddend \citep[GCN
17767;][]{GCN17767}, GRB.
...
12.4 minutes after the \DIFdelbegin \DIFdel{onboard }\DIFdelend \DIFaddbegin \DIFadd{on-board }\DIFaddend trigger of \swift, a total of $4 \times 600$ s
...
\DIFdelbegin \DIFdel{600s}\DIFdelend \DIFaddbegin \DIFadd{600 s}\DIFaddend , securely allows us to measure a redshift for this GRB. Beginning at $\sim$
...
GRB~160625B is the first GRB for which a \DIFdelbegin \DIFdel{signicant }\DIFdelend \DIFaddbegin \DIFadd{significant }\DIFaddend linear polarization was
...
started 323 hr after the burst trigger and were \DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend on the potential host
...
the \lya-line we infer \nh$=20.96 \pm 0.05$. This spectrum will be further \DIFdelbegin \DIFdel{analyzed }\DIFdelend \DIFaddbegin \DIFadd{analysed }\DIFaddend in de Ugarte Postigo et al. (in prep).
...
\lya~forest, with the broad \lya~absorption trough \DIFdelbegin \DIFdel{centered }\DIFdelend \DIFaddbegin \DIFadd{centred }\DIFaddend at 565 nm. In the
